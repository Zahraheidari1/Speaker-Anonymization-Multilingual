...
def prepare_libri(
    ...
    # Also skip if good CSVs already exist
    save_folder_path = Path(save_folder)
    train_csv = save_folder_path / TRAIN_CSV
    dev_csv   = save_folder_path / DEV_CSV
    if train_csv.exists() and dev_csv.exists():
        if train_csv.stat().st_size > 500 and dev_csv.stat().st_size > 500:
            logger.info("CSVs already exist and have content, skipping preparation.")
            return
        ...
        with open(f'{data_folder}/wav.scp', 'r') as f:
            for line in f:
                ...
                if f not in spk_files[used_id]:
                    full_utt += 1
                    spk_files[used_id].append(wav_path)
